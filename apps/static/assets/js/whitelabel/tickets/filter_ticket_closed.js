...
document.getElementById('filter-button').addEventListener('click', function(event) {
    ...
    if (alertShown) {
        return; // Salir de la función si ya se ha mostrado una alerta
    }
    const dateFromInput = document.getElementById('date_from').value;
    const dateToInput = document.getElementById('date_to').value;

    // Validar si solo se ha seleccionado una fecha
    if ((!dateFromInput && dateToInput) || (dateFromInput && !dateToInput)) {
        alert(translations.confirmDateBoth);
        alertShown = true; // Marcar que se ha mostrado una alerta
        setTimeout(() => { alertShown = false; }, 100);
        return; // Detener la ejecución adicional de la validación si falta una fecha
    }

    // Validar si la fecha "To" es menor que la fecha "From"
    if (dateFromInput && dateToInput) {
        const dateFrom = new Date(dateFromInput);
        const dateTo = new Date(dateToInput);

        if (dateTo < dateFrom) {
            alert(translations.confirmDateEnd);
            alertShown = true; // Marcar que se ha mostrado una alerta
            setTimeout(() => { alertShown = false; }, 100);
            return; // Detener la ejecución adicional de la validación si la fecha "To" es menor que la fecha "From"
        }
    }
    ...
    fetch(`${languagePrefix}/whitelabel/tickets/closed/`, {
        method: 'POST',
        body: formData,
        headers: {
    ...
    .then(data => {
        const rows = document.getElementById('ticket-tbody');
        ...
        if (data.success && data.results.length > 0) {
            data.results.forEach(ticket => {
                ...
                row.innerHTML = `
                    ...
                    <td class="align-middle text-center text-sm">
                        <span class="text-secondary mb-0 utc-date-5s">${ticket.created_at}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="text-secondary mb-0 utc-date-5s">${ticket.last_comment}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="text-secondary mb-0 ">${ticket.days} ${translations.days}, ${ticket.hours} Hrs, ${ticket.minutes} Min</span>
                    </td>
                `;
            ...
            // Función para establecer la página y enviar el formulario
            ...
            // Re-aplicar los event listeners a los nuevos elementos, si es necesario
            attachRatingEventListeners();

            // Asegúrate de inicializar HTMX nuevamente en los nuevos elementos, si estás usando HTMX
            htmx.process(rows);

        } else {
            alert(translations.noResultsFound);
            alertShown = true; // Marcar que se ha mostrado una alerta
            rows.innerHTML = ''; // Limpiar el contenido actual
            paginationContainer.innerHTML = ''; // Limpiar el contenido de la paginación
            paginationInfo.textContent = ''; // Limpiar la información de paginación
            setTimeout(() => { alertShown = false; }, 100);
        }

        // Ocultar el overlay después de recibir los datos
