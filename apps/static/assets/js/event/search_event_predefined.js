...
input.addEventListener("keyup", (e) => {
    ...
    const url = `${languagePrefix}/events/predefined/event-predefined${queryParam}`;
    ...
    fetch(url)
        ...
        .then(data => {
            const rows = document.querySelector("#event-predefined-tbody");
            rows.innerHTML = ''; // Limpiar el contenido actual
            data.results.forEach((event_predefined) => {
                const row = document.createElement("tr");

                row.innerHTML = `
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${event_predefined.number}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${event_predefined.name}</span>
                    </td>
