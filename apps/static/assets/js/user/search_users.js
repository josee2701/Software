...
input.addEventListener("keyup", (e) => {
    ...
    const url = `${languagePrefix}/users/users-user${queryParam}`;
    ...
    fetch(url)
        ...
        .then(data => {
            const rows = document.querySelector("#user-tbody");
            rows.innerHTML = ''; // Limpiar el contenido actual

            data.results.forEach(user => {
                const row = document.createElement("tr");
                const updateUrl = updateUserUrl.replace('0', `${user.id}`);
                const permUrl = updatePermissionUrl.replace('0', `${user.id}`);
                const deleteUrl = deleteUserUrl.replace('0', `${user.id}`);
                let status = user.is_active ? translations.true : translations.false;
                let buttonsHtml = '';
                ...
                buttonsHtml += `<button type="button" hx-get="${permUrl}" class="btn btn-primary btn-sm" hx-target="#modal .modal-content" style="margin-right: 3px;">
                        <i class="fa-solid fa-key"></i>
                      </button>`

                if (permiseDelete === "True" && +userId !== user.id) { // Verifica el permiso de eliminación
                ...
                if (permiseDelete === "True" && +userId === user.id) { // Verifica el permiso de eliminación
                    buttonsHtml += `<button type="button" class="btn btn-danger btn-sm ml-2" style="visibility: hidden; width: 39px; height: 30px; display: inline-block;" hx-get="${deleteUrl}" hx-target="#modal .modal-content"></i></button>`;
                }

                row.innerHTML = `
                    <td style="text-align: center">${buttonsHtml}</td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${user.company}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${user.process}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${user.username}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${user.first_name}</span>
                    </td>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${user.last_name}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${user.email}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="text-secondary mb-0">${status}</span>
                    </td>
