...
input.addEventListener("keyup", (e) => {
    ...
    const url = `${languagePrefix}/realtime/group_vehicles/group-assets${queryParam}`;
    ...
    fetch(url)
        ...
        .then(data => {
            const rows = document.querySelector("#group-tbody");
            rows.innerHTML = ''; // Limpiar el contenido actual

            data.results.forEach(group => {
                const row = document.createElement("tr");
                const updateUrl = updateGroupUrl.replace('0', `${group.id}`);
                const deleteUrl = deleteGroupUrl.replace('0', `${group.id}`);
                ...
                row.innerHTML = `
                    ...
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${group.name}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${group.VehicleCount}</span>
                    </td>
