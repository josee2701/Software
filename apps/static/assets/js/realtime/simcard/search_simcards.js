...
input.addEventListener("keyup", (e) => {
    ...
    const url = `${languagePrefix}/realtime/simcards/simcards-user${queryParam}`;
    ...
    fetch(url)
        ...
        .then(data => {
            const rows = document.querySelector("#simcard-tbody");
            rows.innerHTML = ''; // Limpiar el contenido actual

            data.results.forEach(simcard => {
                const row = document.createElement("tr");
                const updateUrl = updateSimcardUrl.replace('0', `${simcard.id}`);
                const deleteUrl = deleteSimcardUrl.replace('0', `${simcard.id}`);
                let statusText = '';
                if (simcard.is_active === false) {
                    statusText = translations.false;
                } else if (simcard.is_active === true ) {
                ...
                row.innerHTML = `
                    ...
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${simcard.company}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${simcard.phone_number}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${simcard.serial_number}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${simcard.data_plan}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="text-secondary text font-weight-normal mb-0">${simcard.activate_date}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
