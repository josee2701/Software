document.getElementById('exportExcelBtn').addEventListener('click', async () => {
    ...
    const url = `${languagePrefix}/realtime/simcards/simcards-export${queryParam}`;
    ...
    try {
        ...
        data.data.forEach(item => {
            wsData.push([
                ...
                item.data_plan,
                item.activate_date,
                item.is_active ? translations.true : translations.false,
            ]);
        });

        const ws = XLSX.utils.aoa_to_sheet(wsData);
        const sheetName = translations.simcards || "List_Simcards";
