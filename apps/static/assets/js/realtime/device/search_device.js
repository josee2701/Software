...
input.addEventListener("keyup", (e) => {
    ...
    const url = `${languagePrefix}/realtime/devices/devices-user${queryParam}`;
    ...
    fetch(url)
        ...
        .then(data => {
            const rows = document.querySelector("#device-tbody");
            rows.innerHTML = ''; // Limpiar el contenido actual

            data.results.forEach(device => {
                const row = document.createElement("tr");
                const updateUrl = updateDeviceUrl.replace('0', `${device.imei}`);
                const deleteUrl = deleteDeviceUrl.replace('0', `${device.imei}`);
                let status = device.is_active ? translations.true : translations.false;
                let buttonsHtml = '';
