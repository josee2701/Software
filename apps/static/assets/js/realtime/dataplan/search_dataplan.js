...
input.addEventListener("keyup", (e) => {
    ...
    const url = `${languagePrefix}/realtime/dataplan/dataplan-user${queryParam}`;
    ...
    fetch(url)
        ...
        .then(data => {
            const rows = document.querySelector("#dataplan-tbody");
            rows.innerHTML = ''; // Limpiar el contenido actual

            data.results.forEach(dataplan => {
                const row = document.createElement("tr");
                const updateUrl = updateDataplanUrl.replace('0', `${dataplan.id}`);
                const deleteUrl = deleteDataplanUrl.replace('0', `${dataplan.id}`);
                ...
                row.innerHTML = `
                    ...
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${dataplan.company}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${dataplan.DataPlanName}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${dataplan.operator}</span>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${dataplan.coin}</span>
                    </td>
                    </td>
                    <td class="align-middle text-center text-sm">
                        <span class="font-weight-normal mb-0">${dataplan.price}</span>
                    </td>
