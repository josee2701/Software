...
def fetch_all_event(search_query=None):
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "EXEC [dbo].[ListEnventdByCompany] @SearchQuery=%s",
                [search_query],
            )
...
def fetch_all_event_personalized(company, user, search_query=None):
    try:
        with connection.cursor() as cursor:
            # Asegurarse de que 'company' es del tipo correcto, por ejemplo, un entero
            ...
            # Llamada al procedimiento almacenado con el parámetro correctamente formateado
            cursor.execute(
                "EXEC [dbo].[ListEnventPersonalizedByCompany] @CompanyId=%s, @UserId=%s, @SearchQuery=%s",
                [company_id, user, search_query],
            )
